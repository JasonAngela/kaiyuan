...
function addpostil5() {
	...
	art.dialog({
		id:'inputDialog', 
		title:'添加批注', 
		content:'<textarea id="postil5" rows="10" cols="30">\r\n\r\n\r\n\r\n\r\n\r\n\r\n \r\n'+user+'\r\n'+currentdate+'('+other_range+')'+'</textarea>',
		lock:true
	
	}
	, function(){
		var value = document.getElementById("postil5").value;
		if(!value){
			art.dialog({
...
function loader5(){
	...
	$.each($(".content5 ins"), function(a, b){
		var content = $(b).attr("comment");
		var $postil = $("<div forid5='"+$(b).get(0).id+"'>"+content+"<span onClick='removepostil5(this)'>　　x</span></div>");
		$postil.hover(function(){
			$(this).css("border-color", "red");
			$("#"+$(this).attr("forid5")+"").removeClass().addClass("postilFocus");
		}
		,
		function(){
			$(this).css("border-color", "#ddd");
			$("#"+$(this).attr("forid5")+"").removeClass().addClass("postil");
		});
		$(b).hover(function(){
			$(this).removeClass().addClass("postilFocus");
			$("div[forid5='"+$(this).get(0).id+"']").css("background-color", "#1E90FF");
		}
		,
		function(){
			$(this).removeClass().addClass("postil");
			$("div[forid5='"+$(this).get(0).id+"']").css("background-color", "#C3DAF9");
		});
		$list.append($postil);
	});
	
	var content_value = $("#content_value5");
	content_value.val(($(".content5").html().trim()));
}
//删除批注
function removepostil5(arg){
	var $div = $(arg).parent();
	var id = $div.attr("forid5");
	var $source = $("#"+id);
