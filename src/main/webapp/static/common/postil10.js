/**
 * @作者：WilliamSha
 * @时间：2011-10-19 下午09:92:01
 * @项目名：XXX
 * @描述：操作批注工具
 */
//初始化页面元素
$(function(){
	$(".content9").mouseup(function(e){
		var selectedText ;
...
function addpostil9() {
	...
	art.dialog({
		id:'inputDialog', 
		title:'添加批注', 
		content:'<textarea id="postil9" rows="10" cols="30">\r\n\r\n\r\n\r\n\r\n\r\n\r\n \r\n'+user+'\r\n'+currentdate+'('+other_range+')'+'</textarea>',
		lock:true
	
	}
	, function(){
		var value = document.getElementById("postil9").value;
		if(!value){
			art.dialog({
...
function loader9(){
	...
	$.each($(".content9 ins"), function(a, b){
		var content = $(b).attr("comment");
		var $postil = $("<div forid9='"+$(b).get(0).id+"'>"+content+"<span onClick='removepostil9(this)'>　　x</span></div>");
		$postil.hover(function(){
			$(this).css("border-color", "red");
			$("#"+$(this).attr("forid9")+"").removeClass().addClass("postilFocus");
		}
		,
		function(){
			$(this).css("border-color", "#ddd");
			$("#"+$(this).attr("forid9")+"").removeClass().addClass("postil");
		});
		$(b).hover(function(){
			$(this).removeClass().addClass("postilFocus");
			$("div[forid9='"+$(this).get(0).id+"']").css("background-color", "#1E90FF");
		}
		,
		function(){
			$(this).removeClass().addClass("postil");
			$("div[forid9='"+$(this).get(0).id+"']").css("background-color", "#C3DAF9");
		});
		$list.append($postil);
	});
	
	var content_value = $("#content_value9");
	content_value.val(($(".content9").html().trim()));
}
//删除批注
function removepostil9(arg){
	var $div = $(arg).parent();
	var id = $div.attr("forid9");
	var $source = $("#"+id);
