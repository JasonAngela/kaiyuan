...
function addPostil2() {
	...
	art.dialog({
		id:'inputDialog', 
		title:'添加批注', 
		content:'<textarea id="postil2" rows="10" cols="30">\r\n\r\n\r\n\r\n\r\n\r\n\r\n \r\n'+user+'\r\n'+currentdate+'('+other_range+')'+'</textarea>',
		lock:true
	
	}
	, function(){
		var value = document.getElementById("postil2").value;
		if(!value){
			art.dialog({
...
function loader2(){
	...
	$.each($(".content2 ins"), function(a, b){
		var content = $(b).attr("comment");
		var $postil = $("<div forid2='"+$(b).get(0).id+"'>"+content+"<span onClick='removepostil2(this)'>　　x</span></div>");
		$postil.hover(function(){
			$(this).css("border-color", "red");
			$("#"+$(this).attr("forid2")+"").removeClass().addClass("postilFocus");
		}
		,
		function(){
			$(this).css("border-color", "#ddd");
			$("#"+$(this).attr("forid2")+"").removeClass().addClass("postil");
		});
		$(b).hover(function(){
			$(this).removeClass().addClass("postilFocus");
			$("div[forid2='"+$(this).get(0).id+"']").css("background-color", "#1E90FF");
		}
		,
		function(){
			$(this).removeClass().addClass("postil");
			$("div[forid2='"+$(this).get(0).id+"']").css("background-color", "#C3DAF9");
		});
		$list.append($postil);
	});
	
	var content_value = $("#content_value2");
	content_value.val(($(".content2").html().trim()));
}
//删除批注
function removepostil2(arg){
	var $div = $(arg).parent();
	var id = $div.attr("forid2");
	var $source = $("#"+id);
