...
function addPostil1() {
	...
	art.dialog({
		id:'inputDialog', 
		title:'添加批注', 
		content:'<textarea id="postil1" rows="10" cols="30">\r\n\r\n\r\n\r\n\r\n\r\n\r\n \r\n'+user+'\r\n'+currentdate+'('+other_range+')'+'</textarea>',
		lock:true
	
	}
	, function(){
		var value = document.getElementById("postil1").value;
		if(!value){
			art.dialog({
...
function loader1(){
	...
	$.each($(".content1 ins"), function(a, b){
		var content = $(b).attr("comment");
		var $postil = $("<div forid1='"+$(b).get(0).id+"'>"+content+"<span onClick='removePostil1(this)'>　　x</span></div>");
		$postil.hover(function(){
			$(this).css("border-color", "red");
			$("#"+$(this).attr("forid1")+"").removeClass().addClass("postilFocus");
		}
		,
		function(){
			$(this).css("border-color", "#ddd");
			$("#"+$(this).attr("forid1")+"").removeClass().addClass("postil");
		});
		$(b).hover(function(){
			$(this).removeClass().addClass("postilFocus");
			$("div[forid1='"+$(this).get(0).id+"']").css("background-color", "#1E90FF");
		}
		,
		function(){
			$(this).removeClass().addClass("postil");
			$("div[forid1='"+$(this).get(0).id+"']").css("background-color", "#C3DAF9");
		});
		$list.append($postil);
	});
	
	var content_value = $("#content_value1");
	content_value.val(($(".content1").html().trim()));
}
//删除批注
function removePostil1(arg){
	var $div = $(arg).parent();
	var id = $div.attr("forid1");
	var $source = $("#"+id);
