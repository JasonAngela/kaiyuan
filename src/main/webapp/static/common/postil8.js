/**
 * @作者：WilliamSha
 * @时间：2011-10-18 下午07:72:01
 * @项目名：XXX
 * @描述：操作批注工具
 */
//初始化页面元素
$(function(){
	$(".content7").mouseup(function(e){
		var selectedText ;
...
function addpostil7() {
	...
	art.dialog({
		id:'inputDialog', 
		title:'添加批注', 
		content:'<textarea id="postil7" rows="10" cols="30">\r\n\r\n\r\n\r\n\r\n\r\n\r\n \r\n'+user+'\r\n'+currentdate+'('+other_range+')'+'</textarea>',
		lock:true
	
	}
	, function(){
		var value = document.getElementById("postil7").value;
		if(!value){
			art.dialog({
...
function loader7(){
	...
	$.each($(".content7 ins"), function(a, b){
		var content = $(b).attr("comment");
		var $postil = $("<div forid7='"+$(b).get(0).id+"'>"+content+"<span onClick='removepostil7(this)'>　　x</span></div>");
		$postil.hover(function(){
			$(this).css("border-color", "red");
			$("#"+$(this).attr("forid7")+"").removeClass().addClass("postilFocus");
		}
		,
		function(){
			$(this).css("border-color", "#ddd");
			$("#"+$(this).attr("forid7")+"").removeClass().addClass("postil");
		});
		$(b).hover(function(){
			$(this).removeClass().addClass("postilFocus");
			$("div[forid7='"+$(this).get(0).id+"']").css("background-color", "#1E90FF");
		}
		,
		function(){
			$(this).removeClass().addClass("postil");
			$("div[forid7='"+$(this).get(0).id+"']").css("background-color", "#C3DAF9");
		});
		$list.append($postil);
	});
	
	var content_value = $("#content_value7");
	content_value.val(($(".content7").html().trim()));
}
//删除批注
function removepostil7(arg){
	var $div = $(arg).parent();
	var id = $div.attr("forid7");
	var $source = $("#"+id);
