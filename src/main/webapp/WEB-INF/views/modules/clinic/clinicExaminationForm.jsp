...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/clinic/clinicExamination/form?id=${clinicExamination.id}">人员验伤<shiro:hasPermission name="clinic:clinicExamination:edit">${not empty clinicExamination.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="clinic:clinicExamination:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="clinicExamination" action="${ctx}/clinic/clinicExamination/save" method="post" class="form-horizontal">
		...
		<input type="hidden" name="register.id" value="${registerId}">                         	
			...
			<label class="control-label" style="margin-left: 26px;">人员验伤图片上传</label>
			<div class="controls">
				<form:hidden id="uploud" path="uploud" htmlEscape="false" class="input-xlarge"/>
				<sys:ckfinder input="uploud" type="files" uploadPath="/clinic/clinicExamination" selectMultiple="true"/>
			</div>                      
		</div>
		<div class="form-actions" style="padding:0">
			<shiro:hasPermission name="clinic:clinicExamination:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
