...
<script type="text/javascript">
    ...
    function ${input}Big(obj){
        var url = $(obj).attr("src");
        windowOpen(url,"查看原图",1000,700);
    }
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li><img src=\""+urls[i]+"\"  onclick=\"${input}Big(this)\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 50 : maxWidth}px;max-height:${empty maxHeight ? 50 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
